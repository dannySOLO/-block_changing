...
const char* GAME_LV[8][10] = {
    ...
    {"img_game_2//parkour.bmp", "img_game_2//img_1.bmp", "img_game_2//img_2.bmp", "img_game_2//img_3.bmp", "img_game_2//img_4.bmp",
...
void arrowKey(){
    ...
    if(k.type == SDL_KEYDOWN){
        if (k.key.keysym.sym == SDLK_LEFT) turnLeft();
        if (k.key.keysym.sym == SDLK_RIGHT) turnRight();
        if (k.key.keysym.sym == SDLK_DOWN) turnDown();
        if (k.key.keysym.sym == SDLK_UP) turnUp();
        if (nineRow() != m || nineCol() != n) time_move ++;
    }
    ...
    while(1){
        ...
        if(time_move == 999){
            ...
            while(true){
                ...
                if (k.type == SDL_KEYDOWN && k.key.keysym.sym == SDLK_RIGHT)return;
            }
        ...
        if((k.type == SDL_KEYDOWN && k.key.keysym.sym == SDLK_ESCAPE) || k.type == SDL_QUIT){
            ...
            while(true){
                SDL_PollEvent(&k) ;
                if (k.type == SDL_KEYDOWN && k.key.keysym.sym == SDLK_LEFT) break;
                if (k.type == SDL_KEYDOWN && k.key.keysym.sym == SDLK_RIGHT)return;
            }
        }
